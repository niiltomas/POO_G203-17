...
public class Cup extends Competition{
    ...
    public void addTeam(Team t){
        super.addTeam(t); //afegeix l'equip també a Competition
  ...
  public void generateMatches(){ //generar tots els partits. 
    ...
    public void simulateMatches(){
        ...
        while ((roundTeams.size() != 1)){ //mentre quedi més d'un equip per jugar
            ...
            for (int k=0; k<homeTeam.getPlayers().size();k++){
                Player playerH = homeTeam.getPlayers().get(k);
                if (playerH instanceof Outfielder){
                    playerH.getStats(this);
                    playerH.update(this, cupMatch);
                } 
            }

            for (int z=0; z<awayTeam.getPlayers().size();z++){
                Player playerA = homeTeam.getPlayers().get(z);
                if (playerA instanceof Outfielder){
                playerA.getStats(this);    
                playerA.update(this, cupMatch);
                } 
            }
