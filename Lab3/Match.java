...
public class Match {
    ...
    public void simulateMatch(){ //Simula un únic partit 
        ...
        for (Player playerA : awayTeam.getPlayers()){ //Actualitzem les estadístiques de cada jugador
            if (playerA instanceof Outfielder){
                Outfielder outfielderA = (Outfielder) playerA;
                outfielderA.noTackles = random.nextInt(3);
                outfielderA.noPasses = random.nextInt(11);
                outfielderA.noShots = random.nextInt(5);
                outfielderA.noAssists = random.nextInt(4);
                outfielderA.updateStats(this);
            }
            else{
                Goalkeeper goalkeeperA = (Goalkeeper) playerA;
                goalkeeperA.noSaves = random.nextInt(4);
                goalkeeperA.updateStats(this); //this fa referència a la instància de Match actual (és a dir, el partit Match que s'està simulant)
            }
        }
            for (Player playerH : homeTeam.getPlayers()){
                if (playerH instanceof Outfielder){
                Outfielder outfielderH = (Outfielder) playerH;
                outfielderH.noTackles = random.nextInt(3);
                outfielderH.noPasses = random.nextInt(11);
                outfielderH.noShots = random.nextInt(5);
                outfielderH.noAssists = random.nextInt(4);
                outfielderH.updateStats(this);
                }
            }
        }    
